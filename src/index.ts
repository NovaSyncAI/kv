...
export class VercelKV extends Redis {
  ...
  async *scanIterator(options?: ScanCommandOptions): AsyncIterable<string> {
    ...
    do {
      ...
      [cursor, keys] = await this.scan(cursor, options) as unknown as [number, string[]];
  ...
  async *hscanIterator(
    ...
    do {
      // eslint-disable-next-line no-await-in-loop -- [@vercel/style-guide@5 migration]
      [cursor, items] = await this.hscan(key, cursor, options) as unknown as [number, (number | string)[]];
  ...
  async *sscanIterator(
    ...
    do {
      // eslint-disable-next-line no-await-in-loop -- [@vercel/style-guide@5 migration]
      [cursor, items] = await this.sscan(key, cursor, options) as unknown as [number, (number | string)[]];
      for (const item of items) {
        yield item;
      }
    } while (cursor !== 0);
  ...
  async *zscanIterator(
    ...
    do {
      // eslint-disable-next-line no-await-in-loop -- [@vercel/style-guide@5 migration]
      [cursor, items] = await this.zscan(key, cursor, options) as unknown as [number, (number | string)[]];
      for (const item of items) {
        yield item;
      }
    } while (cursor !== 0);
